    ...
    <div className="w-full rounded-2xl border border-border/60 bg-card/70 p-0 shadow-card backdrop-blur">
      <div className="flex items-center justify-between border-b border-border/40 px-6 py-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Monthly performance with correlated order volume</p>
        </div>
      ...
      <div className="h-72 w-full p-4">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ left: 10, right: 10, top: 10, bottom: 0 }}>
            ...
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255, 215, 170, 0.15)" vertical={false} />
            <XAxis dataKey="month" stroke="rgba(255, 248, 242, 0.6)" tickLine={false} axisLine={{ stroke: "rgba(255, 215, 170, 0.2)" }} />
            <YAxis
              stroke="rgba(255, 248, 242, 0.6)"
              tickFormatter={(value) => `₹${value}L`}
              tickLine={false}
              axisLine={{ stroke: "rgba(255, 215, 170, 0.2)" }}
            />
            <Tooltip
              content={({ active, payload, label }) => {
                ...
                return (
                  <div style={tooltipStyles}>
                    <p className="text-sm font-medium text-gold-light">{label}</p>
                    <div className="mt-2 space-y-1 text-xs">
                      <p>Revenue: <span className="font-semibold">₹{revenue}L</span></p>
                      <p>Orders: <span className="font-semibold">{orders}</span></p>
                    </div>
                  </div>
                );
              }}
            />
            <Area type="monotone" dataKey="revenue" stroke="rgba(255, 215, 170, 0.9)" strokeWidth={2.6} fill="url(#revenueGradient)" />
            <Area type="monotone" dataKey="orders" stroke="rgba(174, 126, 255, 0.75)" strokeWidth={2} fill="url(#ordersGradient)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
