...
export const commissionsListCols = () => {
    ...
    return [
        ...
        },
        {
            ...
            meta: {
                exportValue: (row: any) =>
                    `${row.partnerDetails?.name || "-"} (${row.partnerDetails?.email || "-"})`,
            },
            cell: ({ row }: { row: any }) => {
                return <div className="">
                    <h1>{row.original.partnerDetails?.name || "-"}</h1>
                    <p className="text-xs mt-2">{row.original.partnerDetails?.email || "-"}</p>
                </div>
            },
        },
        {
            ...
            cell: ({ row }: { row: any }) => {
                return <span className="font-medium">{row.original.commissionAmount || "-"}</span>;
            },
        },
        {
            header: "Actions",
